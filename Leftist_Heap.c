...
leaf Single_Merge(leaf left, leaf right);
...
leaf Heap_Merge(leaf left, leaf right)
{
    ...
    if (left->Value < right->Value)
        return Single_Merge(left, right);
    return Single_Merge(right, left);
}
...
leaf Single_Merge(leaf left, leaf right)
{
    ...
    if (left->Left == NULL)
        ...
        left->Right = Heap_Merge(left->Right, right);
        if (left->Left->Npl < left->Right->Npl) {
            temp = left->Left;
            left->Left = left->Right;
            left->Right = temp;
        }
        left->Npl = left->Right->Npl + 1;
    }
